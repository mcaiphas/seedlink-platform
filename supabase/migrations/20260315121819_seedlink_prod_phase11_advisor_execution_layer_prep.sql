...
create trigger trg_advisor_prompt_templates_updated_at
before update on public.advisor_prompt_templates
for each row
execute function public.set_updated_at();

create trigger trg_advisor_profile_model_configs_updated_at
before update on public.advisor_profile_model_configs
for each row
execute function public.set_updated_at();

create trigger trg_advisor_guardrails_updated_at
before update on public.advisor_guardrails
for each row
execute function public.set_updated_at();

create trigger trg_advisor_execution_runs_updated_at
before update on public.advisor_execution_runs
for each row
execute function public.set_updated_at();

create trigger trg_advisor_escalation_rules_updated_at
