...
begin
  if exists (
    ...
    if not exists (
      ...
      if not exists (select 1 from pg_trigger where tgname = 'trg_finance_accounts_updated_at
') then create trigger trg_finance_accounts_updated_at
    ...
    if not exists (
      ...
      if not exists (select 1 from pg_trigger where tgname = 'trg_journal_entries_updated_at
') then create trigger trg_journal_entries_updated_at
    ...
    if not exists (
      ...
      if not exists (select 1 from pg_trigger where tgname = 'trg_journal_entry_lines_updated_at
') then create trigger trg_journal_entry_lines_updated_at
    ...
    if not exists (
      ...
      if not exists (select 1 from pg_trigger where tgname = 'trg_bank_statement_imports_updated_at
