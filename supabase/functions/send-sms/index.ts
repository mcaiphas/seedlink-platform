...
serve(async (req) => {
  try {
    ...
    const from = config.from_number ?? Deno.env.get("TWILIO_SMS_FROM");

    if (!accountSid || !authToken || !from) {
      return new Response(JSON.stringify({ success: false, error: "Missing Twilio SMS config" }), { status: 400, headers: { "Content-Type": "application/json" } });
    }
    ...
    if (!resp.ok) throw new Error(data?.message ?? "SMS send failed");

    return new Response(JSON.stringify({ success: true, provider: "twilio", provider_message_id: data.sid, provider_response: data }), {
      status: 200,
