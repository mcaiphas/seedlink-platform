      ...
      <CardContent className="space-y-6">
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          <div className="space-y-2">
            ...
            <Input type="number" step="0.01" min="0" value={form.shipping_weight_kg} onChange={e => update('shipping_weight_kg', e.target.value)} placeholder="0.00" />
          </div>
          <div className="space-y-2">
            <Label>Length (cm)</Label>
            <Input type="number" step="0.1" min="0" value={form.length_cm} onChange={e => update('length_cm', e.target.value)} placeholder="0" />
          </div>
          <div className="space-y-2">
            <Label>Width (cm)</Label>
            <Input type="number" step="0.1" min="0" value={form.width_cm} onChange={e => update('width_cm', e.target.value)} placeholder="0" />
          </div>
          <div className="space-y-2">
            <Label>Height (cm)</Label>
            <Input type="number" step="0.1" min="0" value={form.height_cm} onChange={e => update('height_cm', e.target.value)} placeholder="0" />
          </div>
        </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            {volumetricWeight !== null && (
              <div className="p-4 rounded-lg bg-muted/50 border text-center">
                ...
                <p className="text-xs text-muted-foreground mt-1">L×W×H / 5000</p>
              </div>
            )}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="flex items-center justify-between p-4 rounded-lg border bg-muted/30">
            <div><Label>Stackable</Label><p className="text-xs text-muted-foreground">Can be stacked during transport</p></div>
            <Switch checked={isStackable} onCheckedChange={v => update('metadata', { ...form.metadata, stackable: v })} />
          </div>
          <div className="flex items-center justify-between p-4 rounded-lg border bg-muted/30">
            <div><Label>Bulk Item</Label><p className="text-xs text-muted-foreground">Requires special handling for bulk loads</p></div>
            <Switch checked={isBulkItem} onCheckedChange={v => update('metadata', { ...form.metadata, bulk_item: v })} />
          </div>
          <div className={`flex items-center justify-between p-4 rounded-lg border ${isDangerousGoods ? 'border-destructive/30 bg-destructive/5' : 'bg-muted/30'}`}>
            <div>
              <Label className="flex items-center gap-1.5">
                {isDangerousGoods && <AlertTriangle className="h-3.5 w-3.5 text-destructive" />}
                Dangerous Goods
              </Label>
              <p className="text-xs text-muted-foreground">Requires special handling and documentation</p>
            </div>
