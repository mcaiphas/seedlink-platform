...
export default function SupplierAgingReport() {
  ...
  useEffect(() => {
    Promise.all([
      supabase.from('suppliers').select('id, supplier_name, supplier_code').eq('is_active', true),
      supabase.from('supplier_invoices').select('id, supplier_id, invoice_date, due_date, total_amount, paid_amount, status, currency_code').not('status', 'in', '("draft","cancelled","paid")'),
    ]).then(([{ data: s }, { data: inv }]) => {
      setSuppliers(s || []);
      setInvoices(inv || []);
      setLoading(false);
    });
  ...
  const agingData = useMemo(() => {
    ...
    return suppliers.map(sup => {
      ...
      if (supInv.length === 0) return null;
      ...
      return { ...sup, ...buckets, total };
    }).filter(Boolean) as any[];
  }, [suppliers, invoices]);

  const filtered = useMemo(() =>
    agingData.filter(d => !search || d.supplier_name.toLowerCase().includes(search.toLowerCase())),
  [agingData, search]);
  ...
  return (
    <PageShell title="Supplier Aging Report" subtitle="Outstanding payables by age" loading={loading}
      ...
      {filtered.length === 0 ? (
        <EmptyState icon={BarChart3} title="No outstanding balances" description="All supplier accounts are current." />
      ) : (
        <Card className="shadow-sm"><CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              {filtered.map(d => (
                <TableRow key={d.id}>
                  <TableCell className="pl-6 font-medium">{d.supplier_name}</TableCell>
