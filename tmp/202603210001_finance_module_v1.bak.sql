...
-- 2. Read current tenant/company id from JWT claims.
...
-- 3. Check if current JWT has one of the finance/admin roles.
-- Expected JWT examples:
--   app_metadata.roles = ["superuser", "admin", "finance_admin"]
-- or role = "admin"
...
create index if not exists idx_credit_control_customer_name_trgm
  on public.credit_control using gin (customer_name gin_trgm_ops);
...
create policy credit_control_select_policy
on public.credit_control
for select
using (
  public.finance_can_read()
  and tenant_id = public.current_company_id()
);

create policy credit_control_insert_policy
on public.credit_control
for insert
with check (
  public.finance_can_write()
  and tenant_id = public.current_company_id()
);

create policy credit_control_update_policy
on public.credit_control
for update
using (
  public.finance_can_write()
...
create policy credit_control_delete_policy
on public.credit_control
for delete
using (
  public.finance_can_write()
...
create table if not exists public.customer_aging (
  ...
  aging_bucket text not null default '0-30'
    check (aging_bucket in ('current', '0-30', '31-60', '61-90', '90+')),
  amount_due numeric(15,2) not null default 0,
  currency_code text not null default 'ZAR',
  status text not null default 'open'
    check (status in ('open', 'partial', 'paid', 'disputed', 'written_off')),
  created_by uuid,
  updated_by uuid,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
...
create index if not exists idx_customer_aging_customer_name_trgm
  on public.customer_aging using gin (customer_name gin_trgm_ops);
create index if not exists idx_customer_aging_invoice_number_trgm
  on public.customer_aging using gin (invoice_number gin_trgm_ops);
...
create policy customer_aging_select_policy
on public.customer_aging
for select
using (
  public.finance_can_read()
  and tenant_id = public.current_company_id()
...
create policy customer_aging_insert_policy
on public.customer_aging
for insert
with check (
  public.finance_can_write()
...
create policy customer_aging_update_policy
on public.customer_aging
for update
using (
  public.finance_can_write()
...
create policy customer_aging_delete_policy
on public.customer_aging
for delete
using (
  public.finance_can_write()
...
create index if not exists idx_supplier_aging_supplier_name_trgm
  on public.supplier_aging using gin (supplier_name gin_trgm_ops);
create index if not exists idx_supplier_aging_invoice_number_trgm
  on public.supplier_aging using gin (invoice_number gin_trgm_ops);
...
create policy supplier_aging_select_policy
on public.supplier_aging
for select
using (
  public.finance_can_read()
  and tenant_id = public.current_company_id()
...
create policy supplier_aging_insert_policy
on public.supplier_aging
for insert
with check (
  public.finance_can_write()
...
create policy supplier_aging_update_policy
on public.supplier_aging
for update
using (
  public.finance_can_write()
...
create policy supplier_aging_delete_policy
on public.supplier_aging
for delete
using (
  public.finance_can_write()
...
create table if not exists public.customer_credit_notes (
  ...
  status text not null default 'draft'
    check (status in ('draft', 'approved', 'issued', 'applied', 'cancelled')),
...
do 45896 begin if exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'customer_credit_notes' and column_name = 'tenant_id') and exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'customer_credit_notes' and column_name = 'credit_note_number') then create unique index if not exists uq_customer_credit_notes_number on public.customer_credit_notes(tenant_id, credit_note_number) where credit_note_number is not null; end if; end 45896;
...
create index if not exists idx_customer_credit_notes_customer_name_trgm
  on public.customer_credit_notes using gin (customer_name gin_trgm_ops);
create index if not exists idx_customer_credit_notes_reference_trgm
  on public.customer_credit_notes using gin (reference gin_trgm_ops);
...
create policy customer_credit_notes_select_policy
...
create policy customer_credit_notes_insert_policy
on public.customer_credit_notes
for insert
with check (
  public.finance_can_write()
...
create policy customer_credit_notes_update_policy
on public.customer_credit_notes
for update
using (
  public.finance_can_write()
...
create policy customer_credit_notes_delete_policy
on public.customer_credit_notes
for delete
using (
  public.finance_can_write()
...
do 45896 begin if exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'supplier_credit_notes' and column_name = 'tenant_id') and exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'supplier_credit_notes' and column_name = 'credit_note_number') then create unique index if not exists uq_supplier_credit_notes_number on public.supplier_credit_notes(tenant_id, credit_note_number) where credit_note_number is not null; end if; end 45896;
...
create index if not exists idx_supplier_credit_notes_supplier_name_trgm
  on public.supplier_credit_notes using gin (supplier_name gin_trgm_ops);
create index if not exists idx_supplier_credit_notes_reference_trgm
  on public.supplier_credit_notes using gin (reference gin_trgm_ops);
...
create policy supplier_credit_notes_select_policy
...
create policy supplier_credit_notes_insert_policy
on public.supplier_credit_notes
for insert
with check (
  public.finance_can_write()
...
create policy supplier_credit_notes_update_policy
on public.supplier_credit_notes
for update
using (
  public.finance_can_write()
...
create policy supplier_credit_notes_delete_policy
on public.supplier_credit_notes
for delete
using (
  public.finance_can_write()
...
create table if not exists public.refunds (
  ...
  reason text,
  refund_method text
    check (refund_method in ('cash', 'eft', 'card', 'wallet', 'store_credit')),
  refund_date date,
  status text not null default 'pending'
    check (status in ('pending', 'approved', 'processed', 'failed', 'cancelled')),
  approved_by uuid,
  processed_by uuid,
  created_by uuid,
  updated_by uuid,
...
do 45896 begin if exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'refunds' and column_name = 'tenant_id') and exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'refunds' and column_name = 'refund_number') then create unique index if not exists uq_refunds_number on public.refunds(tenant_id, refund_number) where refund_number is not null; end if; end 45896;
...
create index if not exists idx_refunds_customer_name_trgm
  on public.refunds using gin (customer_name gin_trgm_ops);
create index if not exists idx_refunds_reference_trgm
  on public.refunds using gin (reference gin_trgm_ops);
...
create policy refunds_insert_policy
on public.refunds
for insert
with check (
  public.finance_can_write()
...
create policy refunds_update_policy
on public.refunds
for update
using (
  public.finance_can_write()
...
create policy refunds_delete_policy
on public.refunds
for delete
using (
  public.finance_can_write()
...
create table if not exists public.supplier_payments (
  ...
  currency_code text not null default 'ZAR',
  payment_method text
    check (payment_method in ('cash', 'eft', 'card', 'cheque', 'wallet')),
...
do 45896 begin if exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'supplier_payments' and column_name = 'tenant_id') and exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'supplier_payments' and column_name = 'payment_number') then create unique index if not exists uq_supplier_payments_number on public.supplier_payments(tenant_id, payment_number) where payment_number is not null; end if; end 45896;
...
create index if not exists idx_supplier_payments_supplier_name_trgm
  on public.supplier_payments using gin (supplier_name gin_trgm_ops);
create index if not exists idx_supplier_payments_reference_trgm
  on public.supplier_payments using gin (reference gin_trgm_ops);
...
create table if not exists public.customer_statements (
  ...
  status text not null default 'generated'
    check (status in ('generated', 'sent', 'reviewed', 'archived')),
  created_by uuid,
  updated_by uuid,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
...
create index if not exists idx_customer_statements_customer_name_trgm
  on public.customer_statements using gin (customer_name gin_trgm_ops);
...
create policy customer_statements_select_policy
...
create policy customer_statements_insert_policy
on public.customer_statements
for insert
with check (
  public.finance_can_write()
...
create policy customer_statements_update_policy
on public.customer_statements
for update
using (
  public.finance_can_write()
...
create policy customer_statements_delete_policy
on public.customer_statements
for delete
using (
  public.finance_can_write()
...
create index if not exists idx_supplier_statements_supplier_name_trgm
  on public.supplier_statements using gin (supplier_name gin_trgm_ops);
...
create policy supplier_statements_select_policy
on public.supplier_statements
for select
using (
  public.finance_can_read()
  and tenant_id = public.current_company_id()
...
create policy supplier_statements_insert_policy
on public.supplier_statements
for insert
with check (
  public.finance_can_write()
...
create policy supplier_statements_update_policy
on public.supplier_statements
for update
using (
  public.finance_can_write()
...
create policy supplier_statements_delete_policy
on public.supplier_statements
for delete
using (
  public.finance_can_write()
...
create table if not exists public.communication_logs (
  ...
  contact_type text not null
    check (contact_type in ('email', 'sms', 'call', 'whatsapp')),
...
create index if not exists idx_communication_logs_recipient_trgm
  on public.communication_logs using gin (recipient gin_trgm_ops);
create index if not exists idx_communication_logs_subject_trgm
  on public.communication_logs using gin (subject gin_trgm_ops);
...
create policy communication_logs_select_policy
...
create policy communication_logs_insert_policy
on public.communication_logs
for insert
with check (
  public.finance_can_write()
...
create policy communication_logs_update_policy
on public.communication_logs
for update
using (
  public.finance_can_write()
...
create policy communication_logs_delete_policy
on public.communication_logs
for delete
using (
  public.finance_can_write()
...
create index if not exists idx_notification_templates_name_trgm
  on public.notification_templates using gin (name gin_trgm_ops);
create index if not exists idx_notification_templates_code_trgm
  on public.notification_templates using gin (code gin_trgm_ops);
...
create index if not exists idx_operations_finance_reference_trgm
  on public.operations_finance using gin (reference gin_trgm_ops);
create index if not exists idx_operations_finance_description_trgm
  on public.operations_finance using gin (description gin_trgm_ops);
...
create policy operations_finance_select_policy
on public.operations_finance
for select
using (
  public.finance_can_read()
  and tenant_id = public.current_company_id()
...
create policy operations_finance_insert_policy
on public.operations_finance
for insert
with check (
  public.finance_can_write()
...
create policy operations_finance_update_policy
on public.operations_finance
for update
using (
  public.finance_can_write()
...
create policy operations_finance_delete_policy
on public.operations_finance
for delete
using (
  public.finance_can_write()
...
create table if not exists public.inventory_valuation (
  ...
  valuation_method text not null default 'weighted_average'
    check (valuation_method in ('fifo', 'lifo', 'weighted_average', 'standard')),
...
create index if not exists idx_inventory_valuation_product_name_trgm
  on public.inventory_valuation using gin (product_name gin_trgm_ops);
create index if not exists idx_inventory_valuation_sku_trgm
  on public.inventory_valuation using gin (sku gin_trgm_ops);
...
create policy inventory_valuation_select_policy
...
create policy inventory_valuation_insert_policy
on public.inventory_valuation
for insert
with check (
  public.finance_can_write()
...
create policy inventory_valuation_update_policy
on public.inventory_valuation
for update
using (
  public.finance_can_write()
...
create policy inventory_valuation_delete_policy
on public.inventory_valuation
for delete
using (
  public.finance_can_write()
...
create index if not exists idx_commerce_accounting_reference_trgm
  on public.commerce_accounting using gin (reference gin_trgm_ops);
...
create policy commerce_accounting_select_policy
...
create policy commerce_accounting_insert_policy
on public.commerce_accounting
for insert
with check (
  public.finance_can_write()
...
create policy commerce_accounting_update_policy
on public.commerce_accounting
for update
using (
  public.finance_can_write()
...
create policy commerce_accounting_delete_policy
on public.commerce_accounting
for delete
using (
  public.finance_can_write()
